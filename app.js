...
/* ===== v1.3.6 deterministic loading and synchronisation controller =====
   This final controller deliberately overrides earlier incremental versions.
   User-triggered loads always own the blocking overlay. Background syncs never do. */
state.activeSyncPromise = null;
state.visibleLoadActive = false;
state.loadingDepth = 0;
state.loadingShownAt = 0;
state.lastConnectionPaint = 0;
...
function setLoading(active,text='Loading…'){
  ...
  const remaining=minimumVisibleMs-(Date.now()-Number(state.loadingShownAt||0));
  if(remaining>0)setTimeout(finish,remaining);else finish();
}

function refreshConnectionLabel(){
  if(!state.user)return;
  const last=Number(state.lastSuccessfulSync||0);
  if(!last){updateConnectionState('connecting','Connecting…');return}
  const age=Math.max(0,Date.now()-last);
  const rounded=Math.floor(age/5000)*5;
  if(age>90000){
    updateConnectionState('offline',`Offline · last connected ${new Date(last).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}`);
    return;
  }
  // Background synchronisation must not replace the elapsed-time display.
  // Only a user-visible blocking refresh uses the Updating label.
  if(state.visibleLoadActive)updateConnectionState('connecting','Updating…');
  else updateConnectionState('live',`Live · updated ${rounded}s ago`);
}

async function performSync(){
  const from=localDate(state.viewMode==='day'?state.day:state.weekStart);
  const to=localDate(state.viewMode==='day'?state.day:addDays(state.weekStart,6));
  const seq=++state.syncSequence;
  state.syncInFlight=true;
  try{
    const r=await api('sync',{from,to,lastRevision:state.dataRevision||''});
    applySyncResponse(r,seq);
    state.lastSilentSync=Date.now();
    return r;
  } finally {
...
async function loadData(options={}){
  ...
  const label=options.label||'Updating Trade Counter information…';

  if(silent){
    // A blocking user operation always takes priority over background traffic.
    if(state.visibleLoadActive)return;
    if(state.activeSyncPromise)return state.activeSyncPromise;
    state.activeSyncPromise=performSync()
      .catch(e=>{refreshConnectionLabel();return null})
      .finally(()=>{state.activeSyncPromise=null});
  ...
  refreshConnectionLabel();
  try{
    // Allow the overlay to be painted before waiting on any existing request.
    await nextPaint();
    if(state.activeSyncPromise){
      try{await state.activeSyncPromise}catch(_e){}
    }
    // Always fetch the newly selected period after any old background request.
    state.activeSyncPromise=performSync();
    return await state.activeSyncPromise;
  } catch(e){
    showToast(e.message||'The information could not be loaded.','error');
    throw e;
  } finally {
    state.activeSyncPromise=null;
    setLoading(false);
...
function startLiveSync(){
  stopLiveSync();
  const interval=Math.max(20000,Number(CFG.SYNC_INTERVAL_MS||25000));
  state.liveSyncTimer=setInterval(()=>{
    if(!state.user||state.visibleLoadActive||state.activeSyncPromise)return;
    if(document.hidden&&Date.now()-Number(state.lastSilentSync||0)<60000)return;
    loadData({silent:true});
  },interval);
  if(state.connectionTimer)clearInterval(state.connectionTimer);
  refreshConnectionLabel();
  state.connectionTimer=setInterval(refreshConnectionLabel,5000);
}

async function changePlannerPeriod(mutator,label){
...
$('viewMode').onchange=()=>changePlannerPeriod(
  ()=>{state.viewMode=$('viewMode').value},
  'Changing calendar view…'
);
$('prevBtn').onclick=()=>changePlannerPeriod(
  ()=>{if(state.viewMode==='day')state.day=addDays(state.day,-1);else state.weekStart=addDays(state.weekStart,-7)},
  state.viewMode==='day'?'Loading previous day…':'Loading previous week…'
);
$('nextBtn').onclick=()=>changePlannerPeriod(
  ()=>{if(state.viewMode==='day')state.day=addDays(state.day,1);else state.weekStart=addDays(state.weekStart,7)},
  state.viewMode==='day'?'Loading next day…':'Loading next week…'
);
$('todayBtn').onclick=()=>changePlannerPeriod(
  ()=>{state.day=operationalNow();state.weekStart=startOfWeek(operationalNow())},
  'Loading today…'
);
$('refreshBtn').onclick=()=>{closeTransientPanels();loadData({label:'Refreshing Trade Counter information…'})};
...
async function setPage(p){
  ...
  const titles={dashboard:['Dashboard','Today’s operational overview and team workload'],advance:['Advance Picks','View and manage timed large-order collections'],small:['Small Orders','Manage small orders and walk-in collections'],board:['Picking Board','Prioritise and progress all picking work'],reports:['Reports','Customer service, picking and sales performance'],settings:['Settings','Manage your profile and application settings']};
  $('pageTitle').textContent=titles[p][0];$('pageSubtitle').textContent=titles[p][1];
  $('plannerControls').classList.toggle('hidden',['dashboard','reports','settings'].includes(p));
  $('stats').classList.toggle('hidden',['settings','reports'].includes(p));
  if(p==='dashboard'||p==='board'){state.day=operationalNow();state.viewMode='day'}
  else if(p==='advance'||p==='small'){state.weekStart=startOfWeek(operationalNow());state.viewMode='week';resetPlannerFilters()}
  if(p==='settings'){openSettings();return}
  if(p==='reports'){renderAll();return}
  await loadData({label:`Opening ${titles[p][0]}…`});
}
document.querySelectorAll('[data-page]').forEach(x=>x.onclick=()=>setPage(x.dataset.page));

// Focus/visibility refreshes remain silent and are debounced by the active request guard.
document.onvisibilitychange=()=>{if(!document.hidden&&state.user&&!state.visibleLoadActive)loadData({silent:true})};
window.onfocus=()=>{if(state.user&&!state.visibleLoadActive)loadData({silent:true})};
